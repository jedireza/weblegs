...
class Router {
    ...
    public static function route(){
        ...
        for($i = count(self::$paths) - 1; $i >= 0; $i--){
            $tmpPaths = array_slice(self::$paths, 0, $i + 1);
            $routepath = join($tmpPaths, "/");
        ...
        if(file_exists(self::$documentRoot ."/". self::$routeFile)){
            require_once(self::$documentRoot ."/". self::$routeFile);
        }
